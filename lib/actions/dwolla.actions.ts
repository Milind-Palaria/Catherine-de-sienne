...
const validStates = [
  ...
  // Regex pattern for US postal codes (both 5-digit and 9-digit formats)
  ...
  export const createDwollaCustomer = async (
    ...
    try {
      ...
      if (!validStates.includes(newCustomer.state)) {
        throw new Error('Invalid state abbreviation');
      }
  
      // Validate the postal code format
      if (!postalCodePattern.test(newCustomer.postalCode)) {
